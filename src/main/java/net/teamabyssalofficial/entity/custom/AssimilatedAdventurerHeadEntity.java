...
public class AssimilatedAdventurerHeadEntity extends Head implements GeoEntity, Evolving, Hunter {


    public static final EntityDataAccessor<Integer> KILLS = SynchedEntityData.defineId(AssimilatedAdventurerHeadEntity.class, EntityDataSerializers.INT);
    private static boolean hunting = false;

    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
    public AssimilatedAdventurerHeadEntity(EntityType<? extends Monster> pEntityType, Level pLevel) {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Monster.createMobAttributes()
                ...
                .add(Attributes.MAX_HEALTH, FightOrDieMutationsConfig.SERVER.mutated_player_head_health.get())
                .add(Attributes.ATTACK_DAMAGE, FightOrDieMutationsConfig.SERVER.mutated_player_head_damage.get())
    ...
    public void tick() {
        if (this.isAlive() && this.getKills() == 5) {
            ...
            this.discard();
            this.EvolveIntoSimAdventurer(this);
        }
        super.tick();
    }
    private void EvolveIntoSimAdventurer(Entity entity) {
        AssimilatedAdventurerEntity assimilatedAdventurerEntity = new AssimilatedAdventurerEntity(EntityRegistry.ASSIMILATED_ADVENTURER.get(), entity.level());
        assimilatedAdventurerEntity.moveTo(entity.getX(),entity.getY(),entity.getZ());
        entity.level().addFreshEntity(assimilatedAdventurerEntity);
    }
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllerO) {
        controllerO.add(
                new AnimationController<>(this, "controllerO", 7, event -> {
                    if (event.isMoving() && !this.isAggressive()) {
                        return event.setAndContinue(RawAnimation.begin().thenLoop("assimilated_adventurer_head_walk"));
                    }
                    else if (event.isMoving() && this.isAggressive() && this.onGround()) {
                        return event.setAndContinue(RawAnimation.begin().thenLoop("assimilated_adventurer_head_target"));
                    }
                    else if (!event.isMoving() && !this.isAggressive()) {
                        return event.setAndContinue(RawAnimation.begin().thenLoop("assimilated_adventurer_head_idle"));
                    }
        ...
        controllerO.add(
                new AnimationController<>(this, "controllerN", 7, event -> {
                    if (this.isHunting()) {
                        return event.setAndContinue(RawAnimation.begin().thenLoop("assimilated_adventurer_head_air"));
                    }
